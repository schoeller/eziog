...
<div class="content-view-full">
    <div class="class-forum-topic">
        ...
        <div class="content-navigator">
            {section show=$previous_topic}
                <div class="content-navigator-previous">
                    <div class="content-navigator-arrow">&laquo;&nbsp;</div><a href={$previous_topic[0].url_alias|ezurl} title="{$previous_topic[0].name|wash}">{'Previous topic'|i18n( 'design/eziog/full/forum_topic' )}</a>
                </div>
            {section-else}
                <div class="content-navigator-previous-disabled">
                    <div class="content-navigator-arrow">&laquo;&nbsp;</div>{'Previous topic'|i18n( 'design/eziog/full/forum_topic' )}
                </div>
            {/section}
            ...
            {section show=$next_topic}
                <div class="content-navigator-next">
                    <a href={$next_topic[0].url_alias|ezurl} title="{$next_topic[0].name|wash}">{'Next topic'|i18n( 'design/eziog/full/forum_topic' )}</a><div class="content-navigator-arrow">&nbsp;&raquo;</div>
                </div>
            {section-else}
                <div class="content-navigator-next-disabled">
                    {'Next topic'|i18n( 'design/eziog/full/forum_topic' )}<div class="content-navigator-arrow">&nbsp;&raquo;</div>
                </div>
            {/section}
        </div>
        ...
        <form method="post" action={"content/action/"|ezurl}>
            <input class="button forum-new-reply" type="submit" name="NewButton" value="{'New reply'|i18n( 'design/eziog/full/forum_topic' )}" />
            <input type="hidden" name="ContentNodeID" value="{$node.node_id}" />
            <input type="hidden" name="ContentObjectID" value="{$node.contentobject_id}" />
            {if $notification_access}
                <input class="button forum-keep-me-updated" type="submit" name="ActionAddToNotification" value="{'Keep me updated'|i18n( 'design/eziog/full/forum_topic' )}" />
            {/if}
        ...
        {section-else}
           <p>
            {"You need to be logged in to get access to the forums. You can do so %login_link_start%here%login_link_end%"|i18n( "design/eziog/full/forum_topic",,
        ...
        {/section}
            ...
            <tr>
                <th class="author">
                    {"Author"|i18n("design/eziog/full/forum_topic")}
                </th>
                <th class="message">
                    {"Message"|i18n("design/eziog/full/forum_topic")}
                 </th>
            </tr>
            ...
            <tr class="bglight">
               ...
               {let owner=$node.object.owner owner_map=$owner.data_map}
                   ...
                   {section show=is_set( $owner_map.location )}
                       <p>{"Location"|i18n( "design/eziog/full/forum_topic" )}: {$owner_map.location.content|wash}</p>
                   {/section}
                   <p>
                      {let owner_id=$node.object.owner_id}
                          {section var=author loop=$node.object.author_array}
                              {section show=eq($owner_id,$author.contentobject_id)|not()}
                                  {"Moderated by"|i18n( "design/eziog/full/forum_topic" )}: {$author.contentobject.name|wash}
                              {/section}
                          {/section}
                      {/let}
                   </p>
                  ...
                  {section show=$node.object.can_edit}
                      <form method="post" action={"content/action/"|ezurl}>
                          ...
                          <input class="button forum-account-edit" type="submit" name="EditButton" value="{'Edit'|i18n('design/eziog/full/forum_topic')}" />
                  ...
                  {section show=$node.object.can_remove}
                      <form method="post" action={"content/action/"|ezurl}>
                          <input type="hidden" name="ContentObjectID" value="{$node.object.id}" />
                          <input type="hidden" name="ContentNodeID" value="{$node.node_id}" />
                          <input class="button" type="submit" name="ActionRemove" value="{'Remove'|i18n( 'design/eziog/full/forum_topic' )}" title="{'Remove this item.'|i18n( 'design/eziog/full/forum_topic' )}" />                      </form>
                  {/section}
                ...
                <tr class="{$reply.sequence}">
                    ...
                    {let owner=$reply.object.owner owner_map=$owner.data_map}
                        ...
                        {case match=1}
                            ...
                            <input class="button" type="submit" name="EditButton" value="{'Edit'|i18n('design/eziog/full/forum_topic')}" />
                            <input type="hidden" name="ContentObjectLanguageCode" value="{ezini( 'RegionalSettings', 'ContentObjectLocale', 'site.ini')}" />
                            </form>
                        {/case}
                        {case match=0}
                        ...
                        {section show=$node.object.can_remove}
                            <form method="post" action={"content/action/"|ezurl}>
                                ...
                                <input class="button" type="submit" name="ActionRemove" value="{'Remove'|i18n( 'design/eziog/full/forum_topic' )}" title="{'Remove this item.'|i18n( 'design/eziog/full/forum_topic' )}" />
                                </form>
                        {/section}

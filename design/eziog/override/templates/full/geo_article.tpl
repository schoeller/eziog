...
<div class="content-view-full">
    ...
    {if $node.data_map.tags.content.related_objects}
        <div class="attribute-relatedcontent">
            <h1>{"Related content"|i18n("design/eziog/full/article")}</h1>
            <ul>
    ...
    {if is_unset( $versionview_mode )}
        {if $node.data_map.enable_comments.data_int}
            <h1>{"Comments"|i18n("design/eziog/full/article")}</h1>
            ...
            'contentclass_id', 'comment' ) )}
                <form method="post" action={"content/action"|ezurl}>
                    ...
                    <input class="button new_comment" type="submit" name="NewButton" value="{'New comment'|i18n( 'design/eziog/full/article' )}" />
                </form>
                {else}
                {if ezmodule( 'user/register' )}
                    <p>{'%login_link_startLog in%login_link_end or %create_link_startcreate a user account%create_link_end to comment.'|i18n( 'design/eziog/full/article', , hash( '%login_link_start', concat( '<a href="', '/user/login'|ezurl(no), '">' ), '%login_link_end', '</a>', '%create_link_start', concat( '<a href="', "/user/register"|ezurl(no), '">' ), '%create_link_end', '</a>' ) )}</p>
                    {else}
                    <p>{'%login_link_startLog in%login_link_end to comment.'|i18n( 'design/eziog/article/comments', , hash( '%login_link_start', concat( '<a href="', '/user/login'|ezurl(no), '">' ), '%login_link_end', '</a>' ) )}</p>
                {/if}
            {/if}
        {/if}
    {/if}
    ...
    {if and( ezmodule( 'content/tipafriend' ), $tipafriend_access )}
        <div class="attribute-tipafriend">
            <p><a href={concat( "/content/tipafriend/", $node.node_id )|ezurl} title="{'Tip a friend'|i18n( 'design/eziog/full/article' )}">{'Tip a friend'|i18n( 'design/eziog/full/article' )}</a></p>
        </div>
    {/if}
